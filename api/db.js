...
module.exports = function(deps) {
  ...
  return {
    createOrUpdateCard: function(cardBody, deckId, cb) {
      if (typeof cardBody._id === 'undefined') {
        ...
        card
          ...
          .catch(err => {
            logger.info(
              'createCard error in card.save().then((response)' +
                '\n' +
                err.message
            );
            // res.status(400).send('NOT OK');
      ...
      return new Promise((resolve, reject) => {
        if (deckBody._id) {
          ...
          newDeck
            .save()
            .then(savedDeck => {
              User.findOne({ _id: deckBody.owner })
                .then(foundUser => {
    ...
    deleteDeck(deckId) {
      return new Promise((resolve, reject) => {
        Deck.findOne({ _id: deckId }).then(foundDeck => {
          let cardsToDelete = foundDeck.cards.map(deckId => ({
            _id: deckId
          }));

          foundDeck.remove().then(() => {
